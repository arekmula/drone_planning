...
bool Planner3D::checkApproximateSolution(ompl::base::ProblemDefinition* pdef, double errorThreshold)
{
    /*
     * errorThreshold is acceptable distance between approximated result and goal result
     */
    double solutionDistance = pdef->getSolutionDifference();
    std::cout << "Solution distance: " << solutionDistance << "\n";
    if (solutionDistance < errorThreshold)
        return true;
    else
        return false;
}
...
nav_msgs::Path Planner3D::planPath()
{
    ...
    /// clear previous start states, goal and solution paths
    pdef->clearStartStates();
    pdef->clearGoal();
    pdef->clearSolutionPaths();
    /// set new start and goal states for the problem definiton
    pdef->setStartAndGoalStates(*start.get(),*goal.get());

    /* ************************************************************************************************************
     * If you use multi-query planner you can use clearQuery function to use previous data generated by planner to
     * solve next problem.
     * If you use single-query planner you can't use clearQuery function. You have to use clear function.
     * but this time planner will not be able to use previously generated tree to solve next problem
     * List of available planners: https://ompl.kavrakilab.org/planners.html
     * ************************************************************************************************************/

    // uncomment line below if you use multi-query planner and comment line with clear() function
    planner->clearQuery();
    // uncomment line below if you use single-query planner and comment line above
//    planner->clear();
    ...
    planner->setup();
    /// problem status
    std::cout << "Searching for path until planner find EXACT SOLUTION or acceptable APPROXIMATE SOLUTION" << "\n";
    ompl::base::PlannerStatus solved = planner->ompl::base::Planner::solve(SOLVING_TIME);
    /// wait till planner find EXACT SOLUTION or acceptable APPROXIMATE SOLUTION
    while (solved != ompl::base::PlannerStatus::EXACT_SOLUTION)
    {
        if (solved == ompl::base::PlannerStatus::APPROXIMATE_SOLUTION)
        {
            if (checkApproximateSolution(pdef.get(), 0.5))
                break;
        }
        solved = planner->ompl::base::Planner::solve(SOLVING_TIME);
    }
...
void Planner3D::configure(const octomap_msgs::Octomap& octomapMsg)
{
    ...
    /* **************************************************************************
     *  CHOOSE YOUR FIGHTER BELOW and in declaration of planner
